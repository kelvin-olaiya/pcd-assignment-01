...
procedure distribute() begin
    batch_1:
        batches[1] := <<2, 4, 5, 10, 3, 14>>;
    batch_2:
        batches[2] := <<3, 5, 1, 7, 9, 4>>;
end procedure
...
begin
    set_latch:+
        termination_latch := 2;
    call distribute();
    attending: wait_latch(termination_latch);
    finished: skip;
end process; 
...
\* BEGIN TRANSLATION (chksum(pcal) = "ac54d424" /\ chksum(tla) = "21039a7")
...
batch_1(self) == /\ pc[self] = "batch_1"
                 ...
                 /\ UNCHANGED << counter, mutex, termination_latch, stack, 
...
distribute(self) == batch_1(self) \/ batch_2(self)
...
Update(self) == /\ pc[self] = "Update"
                ...
                /\ UNCHANGED << counter, batches, mutex, termination_latch, 
                                stack, length, batch, current_file >>
...
master == set_latch \/ attending \/ finished
...
Count(self) == /\ pc[self] = "Count"
               ...
               /\ UNCHANGED << counter, batches, mutex, termination_latch, 
                               stack, length, temp, current_file >>
...
CS(self) == /\ pc[self] = "CS"
            ...
            /\ UNCHANGED << counter, batches, mutex, termination_latch, batch, 
                            current_file >>
...
Spec == /\ Init /\ [][Next]_vars
        /\ WF_vars(Next)
        /\ WF_vars(master) /\ SF_vars(set_latch) /\ WF_vars(distribute(0))
